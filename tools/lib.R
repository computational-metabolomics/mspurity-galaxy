...
#This function retrieve the .dat files for xset object
xset_pa_filename_fix <- function(opt, pa, xset=NULL){
  ...
  if (!is.null(opt$mzML_files) && !is.null(opt$galaxy_names)){
    ...
    nsave <- names(pa@fileListMS2)
    old_filenames  <- basename(pa@fileListMS2)
 
    pa@fileListMS2 <- filepaths[match(names(pa@fileListMS2), galaxy_names)]
    names(pa@fileListMS2) <- nsave

    pa@puritydf$filename <- basename(pa@fileListMS2[match(pa@puritydf$filename, old_filenames)])
    pa@grped_df$filename <- basename(pa@fileListMS2[match(pa@grped_df$filename, old_filenames)])
  }

#Ajouter le match !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if(!is.null(xset)){
    if(!all(basename(pa@fileListMS2)==basename(xset@filepaths))){
